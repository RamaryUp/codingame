...
int main()
{
    ...
    for (int i = 0; i < H; i++) {
        char line[L*len+1];
        line[L*len] = '\0';
        for (int idx_letter = 0, idx_line = 0; idx_letter < len; idx_letter++)   {
            char character = text[idx_letter];
            int base_index;
            if(isalpha(character))  {
                base_index = L * (toupper(character) - 'A');
            }
            else    {
                base_index = L * IDX_QUESTION_MARK;
            }
            for(int j = 0; j  < L; j++) {
                line[idx_line++] = dict[i][base_index+j];
            }        
        }
